...
class SimulatorSection(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        self._sol_card = ctk.CTkFrame(right, fg_color=COLORS["green_subtle"],
                                       corner_radius=12,
                                       border_width=1, border_color=COLORS["green"])
        self._sol_card.grid(row=0, column=0, sticky="ew",
                             padx=16, pady=(14, 4))
        ...
        self._sol_lbl = ctk.CTkLabel(
            self._sol_card,
            text="Ejecuta una simulación para ver cómo se llegó al resultado.",
            font=("Arial", 13, "italic"),
            text_color=COLORS["text_sub"],
            wraplength=600, justify="left", anchor="w")
        self._sol_lbl.grid(row=1, column=0, sticky="w",
                            padx=14, pady=(2, 10))
        ...
        ctk.CTkFrame(ch, width=3, height=22,
                     fg_color=COLORS["green"],
        ...
        self._chart_area = ctk.CTkFrame(chart_card, fg_color="#FFFFFF",
                                         corner_radius=10)
        self._chart_area.grid(row=1, column=0, sticky="nsew",
                               padx=14, pady=(8,14))
        ...
        cols = ctk.CTkFrame(table_card, fg_color=COLORS["surface"],
                             corner_radius=6)
        cols.grid(row=1, column=0, sticky="ew", padx=16, pady=(10,0))
        cols.grid_columnconfigure((0,1), weight=1)
        for i, col in enumerate(["TIEMPO (T)", "POBLACIÓN P(t)"]):
        ...
        self._table_body.grid(row=2, column=0, sticky="nsew",
                               padx=16, pady=(0,12))
    ...
    def _on_result(self, modelo, t_vals, p_vals, params):
        self._last_t      = t_vals
        self._last_p      = p_vals
    ...
    def _update_chart(self, modelo, t_vals, p_vals, k=None):
        import numpy as np
        ...
        # Línea principal + relleno
        line, = ax.plot(t_vals, p_vals,
                        color="#22C55E", linewidth=2.5, zorder=4)
        ax.fill_between(t_vals, p_vals, alpha=0.10, color="#22C55E", zorder=2)

        # Puntos en cada paso de tiempo
        ax.scatter(t_vals, p_vals,
                   color="#16A34A", s=22, zorder=5, linewidths=0)
        ...
        ax.grid(True, color="#E2E8F0", linewidth=0.8,
                linestyle="--", zorder=0)
        ax.set_xlim(left=min(t_vals) - 0.5, right=max(t_vals) + 0.5)
        ...
        def _on_move(event):
            ...
            if x is None:
                return
            idx = int(np.argmin(np.abs(t_arr - x)))
            tx, ty = t_arr[idx], p_arr[idx]
            annot.xy = (tx, ty)
            annot.set_text(f"t = {tx:g}\nP = {ty:,.2f}")
    ...
    def _update_table(self, t_vals, p_vals):
        ...
        for i, (t, p) in enumerate(zip(t_vals, p_vals)):
            ...
            row_f = ctk.CTkFrame(self._table_body,
    ...
    def _exportar_csv(self):
        ...
        with open(path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["Tiempo (t)", "Población P(t) [individuos]"])
...
class DashboardView(ctk.CTkFrame):
    ...
    def _build_header(self):
        ...
        logo = _draw_logo_small(tf, size=28, bg=COLORS["card"])
        ...
        if self.on_show_intro:
            ctk.CTkButton(pf, text="ℹ Introducción",
                          ...
                          hover_color=COLORS["bg"],
                          text_color=COLORS["text_sub"],
                          command=self.on_show_intro).pack(
                side="left", padx=(0,12))
